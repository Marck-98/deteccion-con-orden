...
document.addEventListener("DOMContentLoaded", () => {
    ...
    async function processCommand(command) {
        try {
            ...
            if (!token) {
                const response = await fetch('http://13.219.234.91/deteccion-con-orden/api/interpret.php', {
                    method: 'POST',
                    headers: {
            ...
            if (token) {
                const audioResponse = await fetch('http://13.219.234.91/deteccion-con-orden/api/speech.php', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                    },
